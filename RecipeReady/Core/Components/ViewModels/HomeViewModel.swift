...
class HomeViewModel {
    ...
    func refresh(recipes: [Recipe]) {
        // 1. Featured
        // Pick one marked isFeatured, or random
        self.featuredRecipe = recipes.first(where: { $0.isFeatured }) ?? recipes.randomElement()
        
        // 2. Eitan's Kitchen
        self.eitanRecipes = recipes.filter { $0.author?.contains("Eitan") == true }
        
        // 3. Cook This Tonight (Fast recipes? < 30 mins)
        self.tonightRecipes = recipes.filter { ($0.cookingTime ?? 999) <= 30 }
